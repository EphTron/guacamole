...
namespace node {
  ...
  ////////////////////////////////////////////////////////////////////////////////
  DynamicLineNode::DynamicLineNode(std::string const& name,
  ...
  void DynamicLineNode::ray_test_impl(Ray const& ray, int options,
  ...
  ////////////////////////////////////////////////////////////////////////////////
  std::shared_ptr<DynamicGeometryResource> const& DynamicLineNode::get_geometry() const {
    return geometry_;
  }

  ////////////////////////////////////////////////////////////////////////////////
  /* virtual */ void DynamicLineNode::accept(NodeVisitor& visitor) {
    visitor.visit(this);
  }
  ...
  ////////////////////////////////////////////////////////////////////////////////
  std::shared_ptr<Node> DynamicLineNode::copy() const {
    return std::make_shared<DynamicLineNode>(*this);
  }
  ...
  void DynamicLineNode::push_vertex(DynamicGeometry::Vertex const& dynamic_vertex) {
    push_vertex(dynamic_vertex.pos[0], dynamic_vertex.pos[1], dynamic_vertex.pos[2],
                dynamic_vertex.col[0], dynamic_vertex.col[1], 
                dynamic_vertex.col[2], dynamic_vertex.col[3],
                dynamic_vertex.thick);
  };

  ////////////////////////////////////////////////////////////////////////////////
  void DynamicLineNode::enqueue_vertex(float x, float y, float z,
  ...
  ////////////////////////////////////////////////////////////////////////////////
  void DynamicLineNode::push_vertex(float x, float y, float z,
                                  float col_r, float col_g, float col_b, float col_a,
                                  float thickness) {
    if(nullptr != geometry_) {
      DynamicGeometry::Vertex vertex_to_push(x, y, z, 
  ...
  void DynamicLineNode::update_vertex(int vertex_idx, float x, float y, float z,
                     float col_r, float col_g, float col_b, float col_a,
                     float thickness) {
    if(nullptr != geometry_) {
      if (vertex_idx < geometry_->num_occupied_vertex_slots()){
        DynamicGeometry::Vertex vertex_to_update(x, y, z, 
                                      col_r, col_g, col_b, col_a, 
                                      thickness);
        auto line_geometry = std::dynamic_pointer_cast<DynamicLineResource>(geometry_);
  ...
  void DynamicLineNode::forward_queued_vertices() {
    if(nullptr != geometry_) {
      auto line_geometry = std::dynamic_pointer_cast<DynamicLineResource>(geometry_);
      line_geometry->forward_queued_vertices(queued_positions_, 
                                         queued_colors_, 
                                         queued_thicknesses_);
      update_bounding_box();
    }
