  ...
  if (sorted_objects != scene.nodes.end() && sorted_objects->second.size() > 0) {
    ...
    std::sort(sorted_objects->second.begin(), sorted_objects->second.end(),
              [](node::Node* a, node::Node* b) {
                return reinterpret_cast<node::DynamicGeometryNode*>(a)->get_material()->get_shader()
                     < reinterpret_cast<node::DynamicGeometryNode*>(b)->get_material()->get_shader();
              });
      ...
      //select the material shader maps belonging to the current visualization mode:
        ...
        if (current_material_shader != dynamic_geometry_node->get_material()->get_shader()) {
          ...
          if (current_material_shader) {
            ...
            Logger::LOG_WARNING << "DynamicGeometryPass::process(): Cannot find material: "
                                << dynamic_geometry_node->get_material()->get_shader_name() << std::endl;
          }
        //}
