...
std::unordered_map<std::string, std::shared_ptr< ::gua::node::Node> >
    DynamicGeometryLoader::loaded_files_ =
        std::unordered_map<std::string, std::shared_ptr< ::gua::node::Node> >();
...
std::shared_ptr<node::Node> DynamicGeometryLoader::load_geometry(
  ...
  if (searched != loaded_files_.end()) {
    ...
    if (topology_type) {
      ...
      if (flags & DynamicGeometryLoader::NORMALIZE_POSITION ||
          flags & DynamicGeometryLoader::NORMALIZE_SCALE) {
...
std::shared_ptr<node::Node> DynamicGeometryLoader::load(
    std::string const& file_name,
    unsigned flags,
...
std::vector<DynamicGeometryResource*> const DynamicGeometryLoader::load_from_buffer(
...
void DynamicGeometryLoader::apply_fallback_material(
