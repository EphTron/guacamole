  ...
  if (sorted_objects != scene.nodes.end() && sorted_objects->second.size() > 0) {
    ...
    std::sort(sorted_objects->second.begin(), sorted_objects->second.end(),
              [](node::Node* a, node::Node* b) {
                return reinterpret_cast<node::DynamicLineNode*>(a)->get_material()->get_shader()
                     < reinterpret_cast<node::DynamicLineNode*>(b)->get_material()->get_shader();
              });
    ...
    for (auto const& object : sorted_objects->second) {
      ...
      if (current_material_shader != dynamic_line_node->get_material()->get_shader()) {
        ...
        if (current_material_shader) {
          ...
          Logger::LOG_WARNING << "DynamicLinePass::process(): Cannot find material: "
                              << dynamic_line_node->get_material()->get_shader_name() << std::endl;
        }
