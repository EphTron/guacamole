...
DynamicLineResource::DynamicLineResource()
    : DynamicGeometryResource(scm::gl::PRIMITIVE_LINE_LIST)  {
  ...
  */
  ...
  if(dynamic_geometry_ptr_->vertex_reservoir_size != 0) {
    DynamicGeometry::Vertex* data(static_cast<DynamicGeometry::Vertex*>(ctx.render_context->map_buffer(
      dynamic_line_to_update_ptr->vertices, scm::gl::ACCESS_WRITE_INVALIDATE_BUFFER)));
    ...
    dynamic_line_to_update_ptr->vertex_array = ctx.render_device->create_vertex_array(
...
void DynamicLineResource::ray_test(Ray const& ray, int options,
...
void DynamicLineResource::forward_queued_vertices(std::vector<scm::math::vec3f> const& queued_positions,
                                                std::vector<scm::math::vec4f> const& queued_colors,
                                                std::vector<float> const& queued_thicknesses) {
  std::lock_guard<std::mutex> lock(dynamic_geometry_update_mutex_);
  std::dynamic_pointer_cast<DynamicGeometry>(dynamic_geometry_ptr_)->forward_queued_vertices(queued_positions,
                                                                                           queued_colors,
                                                                                           queued_thicknesses);
  compute_bounding_box();
  make_clean_flags_dirty();
...
math::vec3 DynamicLineResource::get_vertex(unsigned int i) const {
  return 
