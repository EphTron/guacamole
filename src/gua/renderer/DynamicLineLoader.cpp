...
std::unordered_map<std::string, std::shared_ptr< ::gua::node::Node> >
    DynamicLineLoader::loaded_files_ =
        std::unordered_map<std::string, std::shared_ptr< ::gua::node::Node> >();
...
void DynamicLineLoader::apply_fallback_material(
...
std::shared_ptr<node::DynamicGeometryNode> DynamicLineLoader::create_geometry_instance(std::shared_ptr<DynamicGeometryImporter> importer, 
  ...
  GeometryDatabase::instance()->add(
          ...
          std::make_shared<DynamicLineResource>(std::make_shared<DynamicGeometry>(DynamicGeometry(*importer->get_dynamic_geometry_object_ptr())), 
            flags & DynamicLineLoader::MAKE_PICKABLE));
          //     DynamicTriangle {*importer->get_dynamic_geometry_object_ptr()},
          //     flags & DynamicLineLoader::MAKE_PICKABLE)
          
  std::shared_ptr<node::DynamicGeometryNode> node_to_return = 
        std::make_shared<node::DynamicLineNode>(node::DynamicLineNode("", desc.unique_key()) );
